...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src, double lidar_spin_freq, LaserScanSetting& setting,
  ...
  double range_min_limit, double range_max_limit);
...
int main(int argc, char **argv) {
  ...
  double range_min_limit = 0.02;
  double range_max_limit = 25.0;
  ...
  node->declare_parameter<double>("range_min_limit", range_min_limit);
  node->declare_parameter<double>("range_max_limit", range_max_limit);
  ...
  node->get_parameter("range_min_limit", range_min_limit);
  node->get_parameter("range_max_limit", range_max_limit);
  if (range_min_limit < 0.0) {
    range_min_limit = 0.0;
  }
  if (range_max_limit <= range_min_limit) {
    RCLCPP_WARN(node->get_logger(), "range_max_limit %.2f <= range_min_limit %.2f, adjusting max to min+0.01",
      range_max_limit, range_min_limit);
    range_max_limit = range_min_limit + 0.01;
  }
  ...
  RCLCPP_INFO(node->get_logger(), "<range_min_limit>: %.3f m", range_min_limit);
  RCLCPP_INFO(node->get_logger(), "<range_max_limit>: %.3f m", range_max_limit);
  ...
  while (rclcpp::ok()) {
    switch (ldlidarnode->GetLaserScanData(laser_scan_points, 1500)){
      case ldlidar::LidarStatus::NORMAL: 
        ...
        ToLaserscanMessagePublish(laser_scan_points, lidar_scan_freq, setting, node, publisher, range_min_limit, range_max_limit);
        break;
...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src,  double lidar_spin_freq, LaserScanSetting& setting,
  ...
  scan_time = (stamp - last_stamp).seconds();
  last_stamp = stamp;
  if (scan_time <= 0.0) {
    scan_time = 1.0 / std::max(1.0, lidar_spin_freq);
  }
  // Adjust the parameters according to the demand
  angle_min = 0;
  angle_max = (2 * M_PI);
  range_min = static_cast<float>(range_min_limit);
  range_max = static_cast<float>(range_max_limit);
  ...
  if (lidar_spin_freq > 0) {
    ...
    if (beam_size <= 1) {
      ...
      output.time_increment = static_cast<float>(scan_time / std::max(1, beam_size - 1));
    }
